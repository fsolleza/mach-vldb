...
use chrono::{format::SecondsFormat, DateTime, NaiveDateTime, Utc};
use common::{
	data::*,
...
use lazy_static::*;
use mach_lib::{Mach, MachReader, MachSnapshotIterator, SourcePartition};
use serde::*;
...
static VEC_READER: Mutex<Option<Arc<Mutex<Vec<(u64, Record)>>>>> =
	Mutex::new(None);
...
pub fn enable_scheduler() {
	let mut connection =
		common::ipc::ipc_client_connect("0.0.0.0:3050").unwrap();
	let request = sched_switch::SchedRequest::Enable;
	let _response: sched_switch::SchedResponse =
		common::ipc::ipc_send(&request, &mut connection).unwrap();
}
...
impl Field {
	pub fn from_str(x: &str) -> Option<Self> {
		if x == "kvop" {
			return Some(Self::KVOp);
		}
		if x == "kvcpu" {
			return Some(Self::KVCpu);
		}
		if x == "kvtid" {
			return Some(Self::KVTid);
		}
		if x == "kvdur" {
			return Some(Self::KVDur);
		}
		if x == "histmax" {
			return Some(Self::HistMax);
		}
		if x == "histcnt" {
			return Some(Self::HistCnt);
		}
...
impl Source {
	pub fn from_str(x: &str) -> Option<Self> {
		if x == "hist" {
			return Some(Self::Hist);
		}
		if x == "kv" {
			return Some(Self::KV);
		}
		if x == "sched" {
			return Some(Self::Sched);
		}
		return None;
	}
}
...
fn do_influx_query(query: String) -> Value {
	...
	let mut connection =
		common::ipc::ipc_client_connect("0.0.0.0:3041").unwrap();
	let request = influx_server::InfluxRequest::Query(query);
	let response: influx_server::InfluxResponse =
		common::ipc::ipc_send(&request, &mut connection).unwrap();
...
impl InfluxSchedComm {
	fn parse_object(v: &serde_json::Value) -> Self {
		let count = v.get("count").unwrap().as_u64().unwrap();
		let comm: String =
			v.get("sched_comm").unwrap().as_str().unwrap().into();
		let time = {
			let time = v.get("time").unwrap().as_str().unwrap();
			let parse_fmt = "%Y-%m-%dT%H:%M:%S";
			let p = NaiveDateTime::parse_from_str(time, parse_fmt).unwrap();
