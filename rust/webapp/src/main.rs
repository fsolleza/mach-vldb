...
use collector::*;
use common::data::{micros_since_epoch, Record};
...
fn cache_merge_update(
	...
	let mut map = match cache.get_mut(query) {
		Some(x) => x,
		None => cache.entry(query.clone()).or_insert_with(Cached::new_empty),
	};
...
async fn get_influx_data(
	line: &ScatterLine,
	min_ts: u64,
	max_ts: u64,
) -> FxHashMap<(u64, Vec<FieldValue>), u64> {
	let now = Instant::now();
	let r = if line.source == "kv"
		&& line.field == "kvdur"
		&& line.agg == "count"
	{
...
async fn influxHandler(msg: Json<ScatterRequest>) -> impl IntoResponse {
	let Json(req) = msg;
	let min_ts = req.min_ts_millis * 1000;
	let max_ts = req.max_ts_millis * 1000;
	...
	for r in result {
		...
		for (k, v) in r.iter() {
			...
			let mut data: Vec<ScatterPoint> = v
				.iter()
				.map(|x| ScatterPoint {
					x: x.x / 1_000,
					y: x.y,
...
async fn machHandler(msg: Json<ScatterRequest>) -> impl IntoResponse {
	let now = Instant::now();
